		...
		try(BaseXClient session=new BaseXClient("localhost",1984, "admin", "abc123")) {
			...
			do {
				...
				case 1://Modificar el valor de un elemento de un XML según un ID
					...
					String query1="for $producto in /productos/producto[id="+id+"]  "
							+ "	return $producto";
					BaseXClient.Query baseXQuery1;
					String productosXML1 = null;
					try {
						//Ejecutamos la query para obtener los productos
						baseXQuery1 = session.query(query1);
					    productosXML1 = baseXQuery1.execute();
					    
					    
					}catch (Exception e) {
						// TODO: handle exception
					}
					if (productosXML1.isEmpty()) {
						...
						case "categoria":
							sc.nextLine();
							System.out.println("Introduce el nuevo valor de 'categoria'");
						...
						case "especificaciones":
							sc.nextLine();
							System.out.println("Introduce la especificación a cambiar");
							...
							try {
							    ...
							    NodeList lista=esp.getChildNodes();
							    
							    Boolean existe=false;
							    for (int i = 0; i < lista.getLength(); i++) {
							    	Node nodo=lista.item(i);
							    	if (nodo.getNodeType() == Node.ELEMENT_NODE) {
						                if(nodo.getNodeName().equals(espec)) {
						                	System.out.println("Introduce el nuevo valor de "+espec);
				...
				case 2://.Eliminar un producto según su ID.
					int idEliminar=pedirInt("Introduce el id del producto a eliminar");
					
					String queryel="for $producto in /productos/producto[id="+idEliminar+"]  "
							+ "	return $producto";
					BaseXClient.Query baseXQueryel;
					String productosXMLel = null;
					try {
						//Ejecutamos la query para obtener los productos
						baseXQueryel = session.query(queryel);
					    productosXMLel = baseXQueryel.execute();
					    	    
					}catch (Exception e) {
						// TODO: handle exception
					}
					if (productosXMLel.isEmpty()) {
						System.out.println("No existe el producto indicado");
					}else {
						ejecutarConsulta(session, "delete node db:get('productos')//producto[id="+idEliminar+"]");
